...
from clippedscion import ClippedScion, UnconstrainedClippedScion
...
def main(run, model_trainbias, model_freezebias, lr, momentum):
    ...
    OPTIMIZER = UnconstrainedClippedScion if hyp['opt']['unconstrained'] else ClippedScion
    ...
    optimizer = OPTIMIZER(parameters, lr=lr, momentum=momentum, rho=hyp['opt']['threshold'])
    optimizer_trainbias = optimizer
    optimizer2_trainbias = OPTIMIZER(norm='BiasRMS', scale=radius, params=[whiten_bias], lr=lr, momentum=momentum)
    ...
    for epoch in range(ceil(epochs)):
        ...
        for inputs, labels in train_loader:
            ...
            for opt, sched in zip(optimizers, schedulers):
                opt.step()
                if hyp['opt']['lr_shcedule'] != 'constant':
                    sched.step()
            current_steps += 1
